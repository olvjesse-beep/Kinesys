...
(function instalarMotor31PunhoMao(){
  ...
  function pontuar(cond,texto,c){
    ...
    if(cond.id==='tunel_carpo'){if(mediano)add(4.8,'distribuição mediana');if(mediano&&/(noite|noturn|dirig|celular|sacud)/.test(t))add(1.5,'comportamento típico');if(labelOnly)score=-5;if(ulnar&&!mediano)score-=3;}
    ...
    if(cond.id==='dequervain'){if(radial&&/(polegar|bebe|abrir pote|potes)/.test(t)&&!trauma)add(4.8,'dor radial + carga do polegar');if(basePolegar&&!radial)score-=1.5;if(trauma)score-=2.5;}
    if(cond.id==='tfcc'){if(ulnarDor&&rotCarga&&!ulnar)add(5,'dor ulnar + rotação/carga');if(ulnar)score-=3;}
