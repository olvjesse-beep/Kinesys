...
function prepararHomeCompacta(){
    ...
    if(antigo&&antigo.tagName!=='BUTTON'){
        ...
        const dialog=document.createElement('dialog');dialog.id='ks-home-pending-dialog';dialog.setAttribute('aria-labelledby','ks-home-pending-title');
    ...
    if(rec&&pend&&rec.parentElement===pend.parentElement){
        if(rec.nextElementSibling!==pend)rec.parentElement.insertBefore(pend,rec.nextSibling);
        rec.parentElement.classList.toggle('ks-home-no-pending',typeof telaPermitida==='function'&&!telaPermitida('tela_home_pendencias'));
    }
