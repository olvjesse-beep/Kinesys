...
function renderizarRelatorioDescontos(aviso='') {
    ...
    lista.innerHTML=`<div class="finance-discount-row header"><span>Data</span><span>Paciente</span><span>Plano</span><span>Pacote</span><span>Cortesia</span><span>Total</span></div>`+enriquecidos.map(({p,d})=>{
        ...
        return `<div class="finance-discount-row"><span>${escapeHTML(formatarDataDesconto(p.criado_em))}</span><span><b>${escapeHTML(p.__paciente_nome||'Paciente')}</b><small>${escapeHTML(p.criado_por||'')}</small></span><span>${escapeHTML(p.nome||'Plano de atendimento')}</span><span>${escapeHTML(moedaBR(d.pacote))}</span><span>${escapeHTML(moedaBR(d.cortesia))}${legacy}</span><span><b>${escapeHTML(moedaBR(d.total))}</b></span></div>`;
    }).join('');
}
