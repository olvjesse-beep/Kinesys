...
const fullStart=app.indexOf('async function obterPacienteCompletoPorId(id)');
const fullEnd=app.indexOf('async function obterPacientesSalvos()',fullStart);
assert.ok(fullStart>=0&&fullEnd>fullStart,'selected full-chart loader block must remain in the main core');
...
console.log('Data Cache integration: lightweight patient index remains a 15s session-scoped cache with mutation invalidation; full charts remain uncached after Phase 4I extraction.');
