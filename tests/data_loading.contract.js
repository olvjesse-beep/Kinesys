...
assert.doesNotMatch(src,/async function renderizarPacientesRecentesHome\(\)/,'lightweight Home browse fallback must not be duplicated in the monolithic core');
assert.match(browse,/async function renderizarPacientesRecentesHome\(\)/,'Home browse fallback must exist in its dedicated module');
...
console.log('Data Loading contract: lightweight index, browse fallback and full-chart read module remain separated across the modular runtime.');
