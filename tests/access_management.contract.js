...
assert.match(admin,/addEventListener\('click',interceptarClique,true\)/,'team reset must intercept the legacy email-reset action in capture phase');
...
assert.match(edge,/\.delete\(\)[\s\S]{0,120}kinesys_sessoes_perfil|kinesys_sessoes_perfil'[\s\S]{0,120}\.delete\(\)/,'profile sessions must be invalidated after an administrative password reset');
