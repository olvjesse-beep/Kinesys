...
const navGlass=fs.readFileSync('styles/navigation_glass-1.0.0.css','utf8');
...
assert.match(ds,/live\.id\s*=\s*'ks_route_status'[\s\S]*live\.className\s*=\s*'kds-visually-hidden'/,'status de rota deve continuar como live region acessível visualmente oculto');
assert.match(navGlass,/#ks_route_status\.kds-visually-hidden[\s\S]*clip-path:\s*inset\(50%\)/,'live region de rota não pode ocupar espaço ou aparecer visualmente');
