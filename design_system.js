...
(function(){
    ...
    let pacienteContextoId = storageGet('kinesys_paciente_contexto');
    ...
    function ensureFeedback(){
        ...
        if(!document.getElementById('ks_dialog')){
            ...
            wrap.innerHTML=`<div class="ks-dialog" role="dialog" aria-modal="true" aria-labelledby="ks_dialog_title" aria-describedby="ks_dialog_message">
                ...
                <div class="ks-dialog-actions"><button type="button" class="btn-secondary" id="ks_dialog_cancel">Cancelar</button><button type="button" class="btn-primary" id="ks_dialog_ok">Confirmar</button></div>
            </div>`;
    ...
    function abrirDialog({titulo='Atenção',mensagem='',confirmar='Confirmar',cancelar='Cancelar',tipo='aviso',apenasOk=false,destrutivo=false}={}){
        ensureFeedback();
    ...
    function resolverDialog(valor){
        ...
        const r=dialogResolve; dialogResolve=null; if(r)r(!!valor);
    ...
    function atualizarPaginaAtiva(id){
        ...
        if(id==='tela_buscar')setTimeout(()=>{renderProntuarioCards();atualizarResumoPacienteContexto();},50);
    ...
    function wrapNavegacao(){
        ...
        const wrapped=function(...args){const r=orig.apply(this,args); const ativo=document.querySelector('.tela.ativa')?.id||args[0]; atualizarPaginaAtiva(ativo); return r;};
    ...
    async function obterPacienteContexto(){
        ...
        try{const l=await window.obterPacientesSalvos();return (l||[]).find(p=>String(p.id)===String(pacienteContextoId))||null;}catch(_){return null;}
    }
    ...
    async function renderProntuarioCards(filtro){
        ...
        try{
            const lista=await window.obterPacientesSalvos();const q=(filtro!==undefined?filtro:document.getElementById('input_busca_paciente')?.value||'').trim().toLowerCase();
            const norm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
            const itens=(lista||[]).filter(p=>!q||norm(p.nome).includes(norm(q))||String(p.cpf||'').includes(q.replace(/\D/g,''))||String(p.telefone||'').replace(/\D/g,'').includes(q.replace(/\D/g,'')));
        ...
        }catch(e){c.innerHTML='<div class="ks-empty">Não foi possível carregar os pacientes.</div>';}
    }
    window.renderProntuarioCards=renderProntuarioCards;
    async function atualizarResumoPacienteContexto(){
        const r=document.getElementById('ks_prontuario_resumo');if(!r)return;const p=await obterPacienteContexto();if(!p){r.hidden=true;return;}r.hidden=false;
