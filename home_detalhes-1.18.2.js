...
async function carregarAtendimentosHojeDetalhes(forcar=false){
    ...
    const trabalho=(async()=>{
        ...
        try{
            ...
            for(const id of ['lista_pacientes_recentes','home_det_atend_lista']){const el=document.getElementById(id);if(el)el.innerHTML=mensagem;}
...
function renderizarResumoAtendimentosHome(){
    ...
    el.innerHTML=itens.length?notaHorarioLegadoHome(itens)+itens.slice(0,5).map(cardAtendimentoHomeDetalhes).join(''):'<div class="ks-home-empty">Nenhum atendimento realizado nas últimas 24 horas.</div>';
...
window.renderizarPendenciasClinicas=async function(){
    ...
    try{
        ...
        c.innerHTML=itens.length?itens.slice(0,5).map(i=>`<button type="button" data-home-paciente="${escaparHomeDetalhes(i.id)}" data-home-target="${escaparHomeDetalhes(i.target)}" class="ks-home-pending-item"><span><strong>${escaparHomeDetalhes(i.n)}</strong><span>${escaparHomeDetalhes(i.p)}</span></span><span aria-hidden="true">→</span></button>`).join(''):'<p class="ks-home-empty">Nenhuma pendência automática encontrada.</p>';
        if(itens.length>5)c.innerHTML+='<button type="button" class="btn-secondary" data-home-all-pending>Ver todas as pendências</button>';
    }catch(err){
        if(revisao!==homePendenciasRevisao||escopo!==escopoHomeAtual())return;
...
function prepararHomeCompacta(){
    ...
    if(rec&&!rec.dataset.homeCompact){
        ...
        const h=rec.querySelector('.card-header');if(h)h.innerHTML='<div><h2>Atendimentos · últimas 24 horas</h2><p>Mais recentes primeiro · <span id="home_activity_total">—</span> realizados</p></div><div class="ks-home-header-actions"><button type="button" class="btn-secondary" data-home-refresh>Atualizar</button><button type="button" class="btn-secondary" data-home-all-activity>Ver todos</button></div>';
    }
    const antigo=document.getElementById('card_pendencias_clinicas');
    if(antigo&&antigo.tagName!=='DETAILS'){
        const det=document.createElement('details');det.id=antigo.id;det.className='ks-home-pending';det.hidden=antigo.hidden;
        const sum=document.createElement('summary');sum.innerHTML='<span>Pendências clínicas</span><span id="home_pending_total" aria-live="polite">Carregando…</span><span class="ks-home-disclosure" aria-hidden="true">⌄</span>';
        ...
        det.append(sum,body);antigo.replaceWith(det);
        if(rec&&det.parentElement===rec.parentElement)rec.parentElement.insertBefore(det,rec);
        sum.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();det.open=!det.open;}});
    }
    if(rec&&typeof telaPermitida==='function')rec.hidden=!telaPermitida('tela_home_atendimentos');
    const pend=document.getElementById('card_pendencias_clinicas');
    if(rec&&pend&&rec.parentElement===pend.parentElement&&pend.nextElementSibling!==rec)rec.parentElement.insertBefore(pend,rec);
    ...
    if(!document.body.dataset.homeActivityBound){
        ...
        document.addEventListener('click',e=>{
            ...
            else if(b.hasAttribute('data-home-all-activity'))abrirDetalheHome('atendimentos');
            else if(b.hasAttribute('data-home-all-pending'))abrirDetalheHome('pendencias');
            else if(b.hasAttribute('data-home-refresh')){carregarAtendimentosHojeDetalhes(true);window.renderizarPendenciasClinicas();}
        });
    }
}
...
if(typeof navegarPara==='function'&&!navegarPara.__homeDetalhesWrapped){
    ...
    const wrapped=function(idTela,...args){const r=base.call(this,idTela,...args);setTimeout(()=>{
        ...
        if(idTela==='tela_home'){const det=document.getElementById('card_pendencias_clinicas');if(det)det.open=false;carregarAtendimentosHojeDetalhes();}
...
setInterval(()=>{if(document.visibilityState==='visible'&&document.getElementById('tela_home')?.classList.contains('ativa'))carregarAtendimentosHojeDetalhes();},60000);
