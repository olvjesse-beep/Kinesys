...
async function abrirNotificacaoAgenda(id) {
    ...
    if (n.agendamento_data) agendaDataSelecionada = new Date(n.agendamento_data + 'T00:00:00');
...
async function mudarSemanaAgenda(delta) {
    agendaDataSelecionada = somarDiasAgenda(agendaDataSelecionada, delta * 7);
...
async function renderizarPainelAgenda({ pularSync = false } = {}) {
    ...
    if (titulo) titulo.textContent = descricaoSemana(inicio, fim);
...
/* --------------------------------------------------------------------
   ...
   -------------------------------------------------------------------- */
async function abrirModalAgendamento(profissionalPre, dataPre, horaPre) {
    if (!_supabase) {
        mostrarFeedbackAgenda('Agenda indisponível: conexão com o banco não inicializada.', 'erro');
        return;
    }
    document.getElementById('ag_id').value = '';
    document.getElementById('modal_agendamento_titulo').textContent = 'Novo agendamento';
    ...
    try {
        ...
        const profissionalInicial = podeVerClinicaToda
            ? (profissionalPre || document.getElementById('agenda_filtro_profissional')?.value || '')
            : String(profissionalProprio?.id || '');
        document.getElementById('ag_profissional_select').value = profissionalInicial;
        document.getElementById('ag_data_input').value = dataPre || formatarDataISO(agendaDataSelecionada);
        ...
        atualizarResumoRecorrencia();
        document.getElementById('ag_procedimento_select').value = '';
...
async function atualizarHorariosDisponiveisModal(horaParaPreSelecionar) {
    ...
    try {
        ...
        const livres = calcularSlotsLivres(profissionalId, dataISO, duracao, agendamentosDia || []);
        if (!livres.length) {
...
async function existeConflitoImediato(profissionalId, dataISO, horaInicio, horaFim) {
    let data = [];
    let error = null;
    if (_supabase) {
    ...
    const ini = horaParaMinutos(horaInicio);
    const fim = horaParaMinutos(horaFim);
    return [...data, ...pendentes].filter(a => statusAgendaOcupaHorario(a.status)).some(a => ini < horaParaMinutos(horaCurta(a.hora_fim)) && fim > horaParaMinutos(horaCurta(a.hora_inicio)));
}
