...
function resposta(req: Request, body: unknown, status = 200) { return new Response(JSON.stringify(body), { status, headers: cors(req) }); }
function texto(valor: unknown, max = 160) { return String(valor ?? "").trim().slice(0, max); }
function digitos(valor: unknown, max = 20) { return texto(valor, max * 2).replace(/\D/g, "").slice(0, max); }
function horaCurta(valor: unknown) { return texto(valor, 8).slice(0, 5); }
function uuidValido(valor: unknown) { return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(texto(valor, 40)); }
function dataValida(valor: unknown) { return /^\d{4}-\d{2}-\d{2}$/.test(texto(valor, 10)); }
function horaValida(valor: unknown) { return /^(?:[01]\d|2[0-3]):[0-5]\d$/.test(horaCurta(valor)); }
function horaParaMinutos(hora: string) { const m = hora.match(/^(\d{2}):(\d{2})$/); return m ? Number(m[1]) * 60 + Number(m[2]) : Number.NaN; }
function minutosParaHora(total: number) { return `${String(Math.floor(total / 60)).padStart(2, "0")}:${String(total % 60).padStart(2, "0")}`; }
function dataISO(data: Date) { return `${data.getUTCFullYear()}-${String(data.getUTCMonth() + 1).padStart(2, "0")}-${String(data.getUTCDate()).padStart(2, "0")}`; }
function dataUTC(iso: string) { const m = iso.match(/^(\d{4})-(\d{2})-(\d{2})$/); return m ? new Date(Date.UTC(Number(m[1]), Number(m[2]) - 1, Number(m[3]))) : null; }
function somarDias(iso: string, dias: number) { const d = dataUTC(iso); if (!d) return ""; d.setUTCDate(d.getUTCDate() + dias); return dataISO(d); }
...
function feriadoAgenda(iso: string) {
  const d = dataUTC(iso); if (!d) return null; const ano = d.getUTCFullYear();
  const fixos = new Map([
    [`${ano}-01-01`, "Confraternização Universal"], [`${ano}-04-21`, "Tiradentes"], [`${ano}-05-01`, "Dia do Trabalho"],
    [`${ano}-07-03`, "Aniversário de Montes Claros"], [`${ano}-09-07`, "Independência do Brasil"], [`${ano}-10-12`, "Nossa Senhora Aparecida"],
    [`${ano}-11-02`, "Finados"], [`${ano}-11-15`, "Proclamação da República"], [`${ano}-11-20`, "Dia Nacional de Zumbi e da Consciência Negra"], [`${ano}-12-25`, "Natal"],
  ]);
  if (fixos.has(iso)) return fixos.get(iso) || "Feriado";
  const pascoa = calcularPascoa(ano), sexta = new Date(pascoa.getTime()), corpus = new Date(pascoa.getTime());
  sexta.setUTCDate(sexta.getUTCDate() - 2); corpus.setUTCDate(corpus.getUTCDate() + 60);
...
function intersectar(a: Janela[], b: Janela[]) {
  ...
  for (const x of a) for (const y of b) {
    const inicio = Math.max(horaParaMinutos(horaCurta(x.hora_inicio)), horaParaMinutos(horaCurta(y.hora_inicio)));
    const fim = Math.min(horaParaMinutos(horaCurta(x.hora_fim)), horaParaMinutos(horaCurta(y.hora_fim)));
    if (Number.isFinite(inicio) && Number.isFinite(fim) && inicio < fim) resultado.push({ hora_inicio: minutosParaHora(inicio), hora_fim: minutosParaHora(fim) });
  }
  return [...new Map(resultado.map((j) => [`${j.hora_inicio}|${j.hora_fim}`, j])).values()].sort((x, y) => x.hora_inicio.localeCompare(y.hora_inicio));
}

async function obterContextoClinica(slug: string) {
  const { data: clinica, error: clinicaError } = await supabase.from("clinicas").select("id,nome,slug,ativa").eq("slug", slug).eq("ativa", true).maybeSingle();
  if (clinicaError) throw clinicaError;
  if (!clinica) return null;
  const { data: config, error: configError } = await supabase.from("configuracoes_agendamento_online")
...
async function catalogoPublico(slug: string) {
  ...
  return {
    ...
    clinica: { nome: texto(clinica.nome, 100), slug: texto(clinica.slug, 80) },
    titulo: texto(config.titulo_publico, 100), descricao: texto(config.descricao_publica, 320), mostrar_valores: !!config.mostrar_valores,
    antecedencia_minima_minutos: Math.max(0, Number(config.antecedencia_minima_minutos) || 0),
    horizonte_dias: Math.max(1, Math.min(MAX_HORIZONTE_DIAS, Number(config.horizonte_dias) || 30)), profissionais, procedimentos,
  };
}
...
async function slotsPublicos(slug: string, profissionalId: string, procedimentoId: string) {
  ...
  const [profRes, procRes, horariosRes, pubRes] = await Promise.all([
    supabase.from("equipe").select("id,nome").eq("clinica_id", clinica.id).eq("id", profissionalId).eq("ativo", true).eq("aparece_na_agenda", true).eq("agendamento_online_ativo", true).maybeSingle(),
    supabase.from("procedimentos").select("id,nome,duracao_minutos,profissionais_ids").eq("clinica_id", clinica.id).eq("id", procedimentoId).eq("ativo", true).eq("agendamento_online_ativo", true).maybeSingle(),
    supabase.from("horarios_atendimento").select("profissional_id,dia_semana,hora_inicio,hora_fim").eq("clinica_id", clinica.id),
    supabase.from("disponibilidade_agendamento_online").select("profissional_id,dia_semana,hora_inicio,hora_fim").eq("clinica_id", clinica.id).eq("profissional_id", profissionalId).eq("ativo", true),
  ]);
  ...
  const permitidos = Array.isArray(procRes.data.profissionais_ids) ? procRes.data.profissionais_ids.map(String) : [];
  if (permitidos.length && !permitidos.includes(String(profissionalId))) return { aberto: true, dias: [] };
  ...
  const inicio = agora.data, fim = somarDias(inicio, horizonte - 1);
  const [bloqRes, agRes] = await Promise.all([
    supabase.from("bloqueios_agenda").select("profissional_id,data,hora_inicio,hora_fim").eq("clinica_id", clinica.id).gte("data", inicio).lte("data", fim),
    supabase.from("agendamentos").select("profissional_id,data,hora_inicio,hora_fim,status").eq("clinica_id", clinica.id).eq("profissional_id", profissionalId).gte("data", inicio).lte("data", fim).neq("status", "cancelado"),
  ]);
  for (const r of [bloqRes, agRes]) if (r.error) throw r.error;

  const horarios = horariosRes.data || [], publicados = pubRes.data || [];
  ...
  for (let offset = 0; offset < horizonte && total < MAX_SLOTS_RESPOSTA; offset++) {
    const diaISO = somarDias(inicio, offset), d = dataUTC(diaISO);
    ...
    for (const janela of janelas) {
      const iniJanela = horaParaMinutos(horaCurta(janela.hora_inicio)), fimJanela = horaParaMinutos(horaCurta(janela.hora_fim));
      for (let ini = iniJanela; ini + duracao <= fimJanela && total < MAX_SLOTS_RESPOSTA; ini += duracao) {
        const fimSlot = ini + duracao, horario = minutosParaHora(ini);
        if (diaISO < limite.data || (diaISO === limite.data && horario < limite.hora)) continue;
        if (!ocupados.some(([oi, of]) => ini < of && fimSlot > oi)) { livres.push({ inicio: horario, fim: minutosParaHora(fimSlot) }); total++; }
      }
    }
    const unicos = [...new Map(livres.map((s) => [`${s.inicio}|${s.fim}`, s])).values()];
    if (unicos.length) dias.push({ data: diaISO, horarios: unicos });
  }
  return { aberto: true, profissional: { id: texto(profRes.data.id, 120), nome: texto(profRes.data.nome, 120) }, procedimento: { id: texto(procRes.data.id, 80), nome: texto(procRes.data.nome, 120), duracao_minutos: duracao }, dias };
}
...
function payloadReservaValido(body: Record<string, unknown>) {
  ...
  const dependente = paciente.dependente === true, telefone = digitos(paciente.telefone, 11), cpf = digitos(paciente.cpf, 11), respTelefone = digitos(paciente.responsavel_telefone, 11);
  ...
  if (telefone.length < 10 || telefone.length > 11 || (cpf && cpf.length !== 11)) return null;
  if (dependente && (texto(paciente.responsavel_nome, 120).length < 3 || texto(paciente.responsavel_parentesco, 60).length < 2 || respTelefone.length < 10)) return null;
  return {
    requestId: texto(body.request_id, 40), profissionalId: texto(body.profissional_id, 120), procedimentoId: texto(body.procedimento_id, 40),
    data: texto(body.data, 10), horaInicio: horaCurta(body.hora_inicio), nome: texto(paciente.nome, 120).replace(/\s+/g, " "), cpf,
    nascimento: texto(paciente.nascimento, 10), telefone, dependente,
    responsavelNome: dependente ? texto(paciente.responsavel_nome, 120).replace(/\s+/g, " ") : "",
    responsavelParentesco: dependente ? texto(paciente.responsavel_parentesco, 60) : "", responsavelTelefone: dependente ? respTelefone : "",
  };
}

async function chaveRateLimit(req: Request, slug: string) {
  const encaminhado = texto(req.headers.get("x-forwarded-for"), 240).split(",")[0]?.trim();
  const ip = encaminhado || texto(req.headers.get("cf-connecting-ip"), 80) || texto(req.headers.get("x-real-ip"), 80);
  if (!ip || !SERVICE_ROLE_KEY) return "";
  if (!hmacKeyPromise) {
  ...
  const assinatura = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(`${slug}|${ip}`));
  return [...new Uint8Array(assinatura)].map((b) => b.toString(16).padStart(2, "0")).join("");
}

async function reservaDentroDoLimite(req: Request, slug: string) {
  const chave = await chaveRateLimit(req, slug);
  if (!chave) return true;
  const { data, error } = await supabase.rpc("kinesys_consumir_limite_agendamento_online", {
    p_chave_hash: chave, p_limite: 8, p_janela_segundos: 900,
  });
...
async function reservarOnline(slug: string, body: Record<string, unknown>) {
  ...
  const { data, error } = await supabase.rpc("kinesys_criar_agendamento_online", {
    p_clinica_slug: slug, p_request_id: reserva.requestId, p_profissional_id: reserva.profissionalId, p_procedimento_id: reserva.procedimentoId,
    p_data: reserva.data, p_hora_inicio: reserva.horaInicio, p_nome: reserva.nome, p_cpf: reserva.cpf, p_nascimento: reserva.nascimento, p_telefone: reserva.telefone,
    p_dependente: reserva.dependente, p_responsavel_nome: reserva.responsavelNome || null, p_responsavel_parentesco: reserva.responsavelParentesco || null,
    p_responsavel_telefone: reserva.responsavelTelefone || null,
  });
  if (!error && data?.ok) return { status: 200, body: { ok: true, agendamento_id: data.agendamento_id, data: data.data, hora_inicio: data.hora_inicio, hora_fim: data.hora_fim } };
  const codigo = texto(error?.code, 16);
  if (codigo === "23P01") return { status: 409, body: { erro: "Esse horário não está mais disponível.", codigo: "HORARIO_INDISPONIVEL" } };
  if (codigo === "22023") return { status: 400, body: { erro: texto(error?.message, 240) || "Revise os dados informados.", codigo: "DADOS_INVALIDOS" } };
  if (codigo === "P0001") return { status: 409, body: { erro: "A disponibilidade da agenda mudou. Atualize os horários e tente novamente.", codigo: "AGENDA_ATUALIZADA" } };
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      const body = await lerBody(req), slug = texto(body.clinica, 80).toLowerCase();
      if (!/^[a-z0-9][a-z0-9-]{1,79}$/.test(slug)) return resposta(req, { erro: "Parâmetros inválidos." }, 400);
      if (body.acao === "slots") {
        const profissionalId = texto(body.profissional_id, 120), procedimentoId = texto(body.procedimento_id, 80);
      ...
      if (body.acao === "reservar") {
        if (!(await reservaDentroDoLimite(req, slug))) return resposta(req, { erro: "Muitas tentativas de agendamento. Aguarde alguns minutos e tente novamente.", codigo: "LIMITE_EXCEDIDO" }, 429);
