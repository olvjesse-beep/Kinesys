/* ========================================================================== 
   KINESYS CLINICAL ENGINE 2.5.0 — DIREÇÃO CLÍNICA ASSISTIDA
   --------------------------------------------------------------------------
   Objetivos:
...
(function kinesysClinicalEngine232(){
    ...
    function construirPainelSegurancaGlobal(contexto){
        ...
        const toggle=document.createElement('button');toggle.type='button';toggle.className='btn-secondary ks-safety-toggle';toggle.setAttribute('aria-haspopup','dialog');toggle.textContent=`Segurança clínica · ${revisada||pistasResolvidas?'revisada':totalPendencias?totalPendencias+' pendência(s)':'revisar'}`;
        ...
        toggle.addEventListener('click',()=>{
            panel.hidden=false;
            window.abrirPainelAvaliacaoKineSys('Segurança clínica',panel,toggle);
        });
        if(globais.length){
        ...
        if(!temPistas){
            ...
            else{box.classList.add('pendente');const note=document.createElement('p');note.textContent='Revise a história e os sinais de segurança antes de confirmar. Ausência de pistas automáticas não exclui riscos clínicos.';const confirm=document.createElement('button');confirm.type='button';confirm.className='btn-primary';confirm.textContent='Confirmar revisão de segurança';confirm.addEventListener('click',()=>{window.fecharPainelAvaliacaoKineSys();window.confirmarSegurancaGlobalKineSys();});panel.append(note,confirm);}
        ...
        regioesSelecionadas().forEach(id=>{
            const regional=window.construirSafetyCompacto(id,BANCO_MAPEAMENTO_CLINICO[id],window.obterEstadoRegiao(id),contexto);
            const summary=regional.querySelector('summary');if(summary)summary.textContent=BANCO_MAPEAMENTO_CLINICO[id].nome+' · '+summary.textContent;
            panel.appendChild(regional);
        });
        window.atualizarPainelSegurancaKineSys?.(panel);
        return box;
    }
    ...
    const modalInvestigacao232={aberto:false,idRegiao:'',categoria:'',expandido:'',mostrarTodosTestes:false,filtro:'',retomarFoco:null,keyHandler:null};
    const MODAL_CATEGORIAS_232={
        rotas:{titulo:'Rotas de exame',subtitulo:'Escolha o próximo caminho de investigação, sem fechar diagnóstico.'},
        clusters:{titulo:'Clusters de investigação',subtitulo:'Padrões de achados para orientar a coleta de dados.'},
        diferenciais:{titulo:'Sinais e sintomas diferenciais',subtitulo:'Eixos alternativos que ajudam a separar comportamentos clínicos.'},
        testes:{titulo:'Testes prioritários',subtitulo:'Registre somente os achados pertinentes ao caso.'},
        catalogo:{titulo:'Biblioteca completa',subtitulo:'Pesquise uma condição ou ferramenta específica quando necessário.'}
    };

    function raizModalInvestigacao232(){
        let root=document.getElementById('ks-investigation-modal-root');
        if(!root){root=document.createElement('div');root.id='ks-investigation-modal-root';root.hidden=true;document.body.appendChild(root);}
        return root;
    }
    function fecharModalInvestigacao232(){
        const root=raizModalInvestigacao232(),foco=modalInvestigacao232.retomarFoco;
        modalInvestigacao232.aberto=false;modalInvestigacao232.idRegiao='';modalInvestigacao232.categoria='';modalInvestigacao232.expandido='';modalInvestigacao232.mostrarTodosTestes=false;modalInvestigacao232.filtro='';root.innerHTML='';root.hidden=true;document.body.classList.remove('ks-clinical-modal-open');
        if(modalInvestigacao232.keyHandler){document.removeEventListener('keydown',modalInvestigacao232.keyHandler);modalInvestigacao232.keyHandler=null;}
        const retorno=foco?.isConnected?foco:document.getElementById('subtela_mapeamento');
        retorno?.focus({preventScroll:true});
    }
    function abrirModalInvestigacao232(idRegiao,categoria,trigger){
        modalInvestigacao232.focoInicial=false;
        modalInvestigacao232.aberto=true;modalInvestigacao232.idRegiao=idRegiao;modalInvestigacao232.categoria=categoria;modalInvestigacao232.expandido='';modalInvestigacao232.mostrarTodosTestes=false;modalInvestigacao232.filtro='';modalInvestigacao232.retomarFoco=trigger||document.activeElement;
        if(!modalInvestigacao232.keyHandler){modalInvestigacao232.keyHandler=e=>{if(e.key==='Escape')fecharModalInvestigacao232();if(e.key==='Tab'){const nodes=Array.from(raizModalInvestigacao232().querySelectorAll('button:not(:disabled),input,select')).filter(n=>n.getClientRects().length);const first=nodes[0],last=nodes[nodes.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last?.focus({preventScroll:true});}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus({preventScroll:true});}}};document.addEventListener('keydown',modalInvestigacao232.keyHandler);}
        document.body.classList.add('ks-clinical-modal-open');renderizarModalInvestigacao232();
    }
    window.fecharModalInvestigacaoKineSys=fecharModalInvestigacao232;

    function resumoCurtoInvestigacao232(item){
        const bruto=String(item?.interpretacao||'').replace(/\s+/g,' ').trim();
        if(!bruto)return 'Use os achados registrados para decidir se este eixo merece investigação adicional.';
        const primeiro=(bruto.match(/^.*?[.!?](?:\s|$)/)||[bruto])[0].trim();
        if(primeiro.length<=190)return primeiro;
        return primeiro.slice(0,187).replace(/\s+\S*$/,'')+'…';
    }
    function dadosCategoriaModal232(idRegiao,contexto,categoria){
        if(categoria==='rotas')return sugerirRotasRegiao(idRegiao,contexto).map(rota=>({...rota,tipo:'rota',chave:'rota::'+rota.id}));
        const candidatos=ordenarHipotesesRegiaoKineSys(idRegiao,contexto);
        if(categoria==='clusters')return candidatos.filter(c=>c.grupo==='cluster').map(c=>({...c,tipo:'item',chave:chaveHipoteseKineSys(c)}));
        if(categoria==='diferenciais')return candidatos.filter(c=>c.grupo==='diferencial').map(c=>({...c,tipo:'item',chave:chaveHipoteseKineSys(c)}));
        if(categoria==='testes')return candidatos.slice(0,6).map(c=>({...c,tipo:'item',chave:chaveHipoteseKineSys(c)}));
        return catalogoRegiao232(idRegiao).sort((a,b)=>String(a.item.nome||'').localeCompare(String(b.item.nome||''),'pt-BR')).map(c=>({...c,tipo:'item',chave:chaveHipoteseKineSys(c)}));
    }
    function textoStatusModal232(status){return status==='positivo'?'Achados compatíveis registrados':status==='negativo'?'Achados atuais não sustentam este eixo':status==='inconclusivo'?'Resultado inconclusivo':'Ainda sem dados suficientes';}
    function criarResultadoModal232(idRegiao,entry,index){
        const estado=window.obterEstadoRegiao(idRegiao),atual=valorAchado232(estado,entry.grupo,entry.item,index)||'',select=document.createElement('select');select.className='ks-modal-result';select.setAttribute('aria-label','Resultado do achado '+(index+1));
        [['','— Registrar achado —'],['positivo','Compatível'],['negativo','Não compatível'],['inconclusivo','Inconclusivo'],['nao_realizado','Não avaliado'],['nao_aplicavel','Não se aplica']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t;o.selected=v===atual;select.appendChild(o);});
        atualizarClasseSelectResultado(select);select.addEventListener('change',()=>{gravarAchado232(estado,entry.grupo,entry.item,index,select.value);marcarFerramentaInvestigada232(estado,entry.grupo,entry.item);atualizarDirecaoPeloExame232(idRegiao,entry.grupo,entry.item);agendarAutosaveKineSys();renderizarMapeamentoRegioes();renderizarModalInvestigacao232();});
        return select;
    }
    function renderizarTestesModal232(idRegiao,entry,holder){
        const estado=window.obterEstadoRegiao(idRegiao),testes=entry.item.testes||[],ordem=[...testes.map((_,i)=>i).filter(i=>!!valorAchado232(estado,entry.grupo,entry.item,i)),...testes.map((_,i)=>i)].filter((i,n,a)=>a.indexOf(i)===n),visiveis=modalInvestigacao232.mostrarTodosTestes?ordem:ordem.slice(0,4);
        const note=document.createElement('p');note.className='ks-modal-note';note.textContent='O resultado informa a investigação e não confirma diagnóstico.';holder.appendChild(note);
        const list=document.createElement('div');list.className='ks-modal-test-list';visiveis.forEach(i=>{const row=document.createElement('div');row.className='ks-modal-test-row';const text=document.createElement('span');text.textContent=testes[i];row.append(text,criarResultadoModal232(idRegiao,entry,i));list.appendChild(row);});holder.appendChild(list);
        if(ordem.length>4){const more=document.createElement('button');more.type='button';more.className='ks-modal-inline-action';more.textContent=modalInvestigacao232.mostrarTodosTestes?'Mostrar somente os prioritários':`Ver todos os ${testes.length} achados`;more.addEventListener('click',()=>{modalInvestigacao232.mostrarTodosTestes=!modalInvestigacao232.mostrarTodosTestes;renderizarModalInvestigacao232();});holder.appendChild(more);}
        const status=statusHipoteseKineSys(idRegiao,entry),ver=document.createElement('div');ver.className='ks-modal-status '+status;ver.textContent=textoStatusModal232(status);holder.appendChild(ver);
        const actions=document.createElement('div');actions.className='ks-modal-item-actions';const choose=document.createElement('button');choose.type='button';choose.className='btn-primary';choose.textContent='Usar como eixo de investigação';choose.addEventListener('click',()=>{selecionarDirecaoHipotese(idRegiao,entry);fecharModalInvestigacao232();});actions.appendChild(choose);holder.appendChild(actions);
    }
    function renderizarModalInvestigacao232(){
        const scrollLista=document.querySelector('#ks-investigation-modal-root .ks-investigation-list')?.scrollTop||0;
        const focoResultado=document.activeElement?.classList.contains('ks-modal-result')?document.activeElement.getAttribute('aria-label'):null;
        const root=raizModalInvestigacao232();root.innerHTML='';if(!modalInvestigacao232.aberto){root.hidden=true;return;}root.hidden=false;
        const idRegiao=modalInvestigacao232.idRegiao,contexto=coletarContextoClinico(),regiao=BANCO_MAPEAMENTO_CLINICO[idRegiao],meta=MODAL_CATEGORIAS_232[modalInvestigacao232.categoria]||MODAL_CATEGORIAS_232.catalogo;
        const overlay=document.createElement('div');overlay.className='ks-investigation-overlay';overlay.addEventListener('click',e=>{if(e.target===overlay)fecharModalInvestigacao232();});
        const dialog=document.createElement('section');dialog.className='ks-investigation-dialog';dialog.setAttribute('role','dialog');dialog.setAttribute('aria-modal','true');dialog.setAttribute('aria-labelledby','ks-investigation-title');
        const head=document.createElement('header');head.className='ks-investigation-dialog-head';const kicker=document.createElement('span');kicker.className='ks-kicker';kicker.textContent='APOIO À INVESTIGAÇÃO';const title=document.createElement('h2');title.id='ks-investigation-title';title.textContent=meta.titulo;const context=document.createElement('p');context.textContent=`${regiao?.nome||idRegiao} · ${meta.subtitulo}`;const close=document.createElement('button');close.type='button';close.className='ks-investigation-close';close.setAttribute('aria-label','Fechar painel');close.textContent='×';close.addEventListener('click',fecharModalInvestigacao232);head.append(kicker,title,context,close);dialog.appendChild(head);
        const notice=document.createElement('div');notice.className='ks-investigation-notice';notice.textContent='O motor organiza possibilidades e próximos passos. A decisão permanece com o fisioterapeuta.';dialog.appendChild(notice);
        if(modalInvestigacao232.categoria==='catalogo'){const search=document.createElement('input');search.type='search';search.className='ks-investigation-search';search.placeholder='Buscar condição ou ferramenta';search.setAttribute('aria-label','Buscar condição ou ferramenta');search.value=modalInvestigacao232.filtro;search.addEventListener('input',()=>{const cursor=search.selectionStart||search.value.length;modalInvestigacao232.filtro=search.value;renderizarModalInvestigacao232();requestAnimationFrame(()=>{const next=document.querySelector('#ks-investigation-modal-root .ks-investigation-search');if(next){next.focus();next.setSelectionRange(cursor,cursor);}});});dialog.appendChild(search);}
        const list=document.createElement('div');list.className='ks-investigation-list';let entries=dadosCategoriaModal232(idRegiao,contexto,modalInvestigacao232.categoria);if(modalInvestigacao232.filtro){const q=removerAcentos(modalInvestigacao232.filtro.trim());entries=entries.filter(e=>removerAcentos([e.item?.id,e.item?.nome,...(e.item?.palavrasChaveHMA||[]),e.nome].join(' ')).includes(q));}
        if(!entries.length){const empty=document.createElement('div');empty.className='ks-modal-empty';empty.textContent='Nenhum item corresponde ao contexto atual. Continue com o exame geral ou refine a busca.';list.appendChild(empty);}
        entries.forEach(entry=>{const key=entry.chave||('rota::'+entry.id),expanded=modalInvestigacao232.expandido===key,card=document.createElement('article');card.className='ks-investigation-item'+(expanded?' expanded':'');const top=document.createElement('div');top.className='ks-investigation-item-top';const info=document.createElement('div');info.className='ks-investigation-item-info';const badge=document.createElement('span');badge.className='ks-investigation-badge';badge.textContent=entry.tipo==='rota'?'ROTA':entry.grupo==='cluster'?'CLUSTER':'DIFERENCIAL';const name=document.createElement('strong');name.textContent=entry.nome||entry.item?.nome||'';info.append(badge,name);const summary=document.createElement('p');summary.textContent=entry.tipo==='rota'?(entry.resumo||'Rota de exame para organizar os próximos achados.'):`Diferencia quando: ${resumoCurtoInvestigacao232(entry.item)}`;info.appendChild(summary);top.appendChild(info);const toggle=document.createElement('button');toggle.type='button';toggle.className='ks-modal-item-toggle';toggle.setAttribute('aria-expanded',String(expanded));toggle.textContent=expanded?'Fechar':'Abrir';toggle.addEventListener('click',()=>{modalInvestigacao232.expandido=expanded?'':key;modalInvestigacao232.mostrarTodosTestes=false;renderizarModalInvestigacao232();});top.appendChild(toggle);card.appendChild(top);
            if(expanded){const body=document.createElement('div');body.className='ks-investigation-item-body';if(entry.tipo==='rota'){const ul=document.createElement('ul');(entry.achados||[]).slice(0,4).forEach(a=>{const li=document.createElement('li');li.textContent=a;ul.appendChild(li);});body.appendChild(ul);const actions=document.createElement('div');actions.className='ks-modal-item-actions';const choose=document.createElement('button');choose.type='button';choose.className=rotasSelecionadasEstado(window.obterEstadoRegiao(idRegiao)).includes(entry.id)?'btn-primary':'btn-secondary';choose.textContent=rotasSelecionadasEstado(window.obterEstadoRegiao(idRegiao)).includes(entry.id)?'Rota selecionada':'Selecionar rota';choose.addEventListener('click',()=>{alternarRota(idRegiao,entry.id);renderizarModalInvestigacao232();});actions.appendChild(choose);if(entry.id!=='geral'){const ignore=document.createElement('button');ignore.type='button';ignore.className='ks-modal-inline-action';ignore.textContent='Ignorar por enquanto';ignore.addEventListener('click',()=>{ignorarRota(idRegiao,entry.id);renderizarModalInvestigacao232();});actions.appendChild(ignore);}body.appendChild(actions);}else{renderizarTestesModal232(idRegiao,entry,body);}card.appendChild(body);}list.appendChild(card);});dialog.appendChild(list);const foot=document.createElement('footer');foot.className='ks-investigation-dialog-foot';const closeFoot=document.createElement('button');closeFoot.type='button';closeFoot.className='btn-secondary';closeFoot.textContent='Fechar painel';closeFoot.addEventListener('click',fecharModalInvestigacao232);foot.appendChild(closeFoot);dialog.appendChild(foot);overlay.appendChild(dialog);root.appendChild(overlay);list.scrollTop=scrollLista;requestAnimationFrame(()=>{const alvo=focoResultado?Array.from(root.querySelectorAll('.ks-modal-result')).find(el=>el.getAttribute('aria-label')===focoResultado):null;(alvo||close).focus({preventScroll:true});});
    }

    function construirRotasRegiao(idRegiao,contexto,estado){
        const wrap=document.createElement('section');wrap.className='ks-route-wrap';const ativas=rotasSelecionadasEstado(estado),sugestoes=sugerirRotasRegiao(idRegiao,contexto);
        const head=document.createElement('div');head.className='ks-route-head';const copy=document.createElement('div');copy.innerHTML='<span class="ks-kicker">PRÓXIMO PASSO</span><strong>O que vale examinar agora?</strong><p>Abra somente as rotas necessárias para organizar a investigação.</p>';const open=document.createElement('button');open.type='button';open.className='btn-secondary ks-open-investigation';open.textContent=`Rotas de exame (${sugestoes.length})`;open.addEventListener('click',()=>abrirModalInvestigacao232(idRegiao,'rotas',open));head.append(copy,open);wrap.appendChild(head);
        if(ativas.length){const selected=document.createElement('div');selected.className='ks-selected-routes';const label=document.createElement('span');label.textContent='Selecionadas';selected.appendChild(label);ativas.forEach(id=>{const rota=rotaParaRegiao(idRegiao,id);if(!rota)return;const chip=document.createElement('span');chip.className='ks-selected-route';chip.textContent=rota.nome;selected.appendChild(chip);});wrap.appendChild(selected);}
        return wrap;
    }

    function construirFerramentasEspecificas(idRegiao,contexto,estado){
        const catalogo=catalogoRegiao232(idRegiao);if(!catalogo.length)return null;const candidatos=ordenarHipotesesRegiaoKineSys(idRegiao,contexto),wrap=document.createElement('section');wrap.className='ks-investigation-actions';
        const head=document.createElement('div');head.className='ks-investigation-actions-head';head.innerHTML='<div><span class="ks-kicker">ABRIR QUANDO PRECISAR</span><strong>Conteúdo de investigação</strong><p>Escolha uma categoria. Os detalhes ficam em uma janela única para preservar o foco.</p></div>';wrap.appendChild(head);
        const grid=document.createElement('div');grid.className='ks-investigation-actions-grid';const configs=[['clusters','Clusters',candidatos.filter(c=>c.grupo==='cluster').length],['diferenciais','Sinais diferenciais',candidatos.filter(c=>c.grupo==='diferencial').length],['testes','Testes prioritários',Math.min(6,candidatos.length)],['catalogo','Biblioteca completa',catalogo.length]];
        configs.forEach(([cat,label,count])=>{const b=document.createElement('button');b.type='button';b.className='ks-investigation-action';b.disabled=!count;b.innerHTML=`<span>${escapeHTML(label)}</span><small>${count?`${count} opção(ões)`:'Sem itens disponíveis'}</small><b> abrir</b>`;b.addEventListener('click',()=>abrirModalInvestigacao232(idRegiao,cat,b));grid.appendChild(b);});wrap.appendChild(grid);return wrap;
    }
    ...
    window.renderizarMapeamentoRegioes = function(){
        ...
        const overview=document.createElement('details');overview.className='ks-evaluation-overview';const summary=document.createElement('summary');summary.textContent='Visão geral da investigação';overview.append(summary,construirPainelDirecao(contexto));container.appendChild(overview);
        const regioes=regioesSelecionadas();
        window.atualizarRegioesCompactasKineSys?.(regioes);
        if(!regioes.length){const vazio=document.createElement('div');vazio.className='ks-exam-empty';vazio.innerHTML='<strong>Região não definida</strong><span></span>';const b=document.createElement('button');b.type='button';b.className='btn-primary';b.textContent='Ir para resumo clínico';b.addEventListener('click',window.avancarParaDiagnostico);vazio.appendChild(b);container.appendChild(vazio);container.appendChild(construirPainelSegurancaGlobal(contexto));if(modalInvestigacao232.aberto)fecharModalInvestigacao232();return;}
        const ativa=window.atualizarRegioesCompactasKineSys?.(regioes)||regioes[0];
        container.appendChild(window.construirCardRegiao(ativa,contexto));
        ...
        regioes.forEach(id=>{window.obterEstadoRegiao(id).ferramentaScrollPendente='';});
    };
    ...
    window.coletarAchadosLaudoKineSys = function(){
        return regioesSelecionadas().flatMap(id=>catalogoRegiao232(id).map(entry=>{
            const estado=window.obterEstadoRegiao(id),a=analisarRespostasItem(estado,entry.grupo,entry.item);
            if(!a.respondidos)return null;
            return {regiao:BANCO_MAPEAMENTO_CLINICO[id].nome,nome:entry.item.nome,status:statusHipoteseKineSys(id,entry),completo:a.respostas.length>0&&a.decisivos===a.respostas.length,achados:(entry.item.testes||[]).map((texto,i)=>({texto,resultado:a.respostas[i]||'nao_avaliado'}))};
        }).filter(Boolean));
    };
    ...
    window.KineSysClinicalEngine250=api232;
