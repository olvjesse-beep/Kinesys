...
function calcularResumoBalanco(dados) {
    ...
    const receita=pagamentos.reduce((s,x)=>s+Math.max(0,Number(x.valor)||0),0);
    ...
    return {receita,gasto,resultado:receita-gasto,desc,pagamentos:pagamentos.length,ticket:pagamentos.length?receita/pagamentos.length:0,planos:planos.length,atendidos,faltas,despesas:despesas.length};
}
