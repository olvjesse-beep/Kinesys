...
function irParaSubtela(idSubtela) {
    ...
    if (idSubtela === 'subtela_mapeamento' && alvo) {
...
const KINESYS_MOTOR_VERSION = '2.3.1';
const KINESYS_APP_VERSION = '1.15.0';
...
function coletarClinicaEstruturada(){return{medidasObjetivas:coletarMedidasObjetivas(),psfs:coletarPSFS(),outcomes:coletarOutcomes(),objetivosPlano:coletarObjetivosPlano(),restricoesPosOperatorias:coletarRestricoesPosOperatorias()};}
function limparClinicaEstruturada(){['lista_medidas_objetivas','lista_psfs','lista_outcomes','lista_objetivos_terapeuticos'].forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML='';});['plano_terapeutico','plano_frequencia','plano_criterios','criterios_alta','previsao_alta','posop_procedimento','posop_data_cirurgia','posop_ortese','posop_adm','posop_proibicoes','posop_orientacoes','posop_retorno_medico','posop_obs'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});const ativo=document.getElementById('posop_ativo');if(ativo)ativo.checked=false;const carga=document.getElementById('posop_carga');if(carga)carga.value='';const prot=document.getElementById('posop_protocolo');if(prot)prot.value='nao_informado';atualizarVisibilidadeRestricoesPosOp();atualizarPSFSMedia();}
function preencherClinicaEstruturada(d={}){limparClinicaEstruturada();(d.medidasObjetivas||[]).forEach(adicionarMedidaObjetiva);(d.psfs?.itens||[]).forEach(adicionarAtividadePSFS);(d.outcomes||[]).forEach(adicionarOutcome);(d.objetivosPlano?.objetivos||[]).forEach(adicionarObjetivoTerapeutico);if(document.getElementById('plano_terapeutico'))document.getElementById('plano_terapeutico').value=d.objetivosPlano?.plano||'';if(document.getElementById('plano_frequencia'))document.getElementById('plano_frequencia').value=d.objetivosPlano?.frequencia||'';if(document.getElementById('plano_criterios'))document.getElementById('plano_criterios').value=d.objetivosPlano?.criterios||'';if(document.getElementById('criterios_alta'))document.getElementById('criterios_alta').value=d.objetivosPlano?.criteriosAlta||'';if(document.getElementById('previsao_alta'))document.getElementById('previsao_alta').value=d.objetivosPlano?.previsaoAlta||'';preencherRestricoesPosOperatorias(d.restricoesPosOperatorias||{});if(!(d.medidasObjetivas||[]).length)adicionarMedidaObjetiva();if(!(d.psfs?.itens||[]).length)adicionarAtividadePSFS();if(!(d.objetivosPlano?.objetivos||[]).length)adicionarObjetivoTerapeutico();atualizarPSFSMedia();}
...
function coletarRascunhoKineSys() {
    ...
    return {versaoMotor:KINESYS_MOTOR_VERSION, salvoEm:new Date().toISOString(), pacienteAtualId, usuarioChave:chaveUsuarioRascunhoKineSys(), campos, estadoMapeamento:JSON.parse(JSON.stringify(estadoMapeamento))};
}
...
async function descartarRascunhoAvaliacaoKineSys() {
    const confirmado = await confirmarKineSys(
        'Descartar o rascunho automático e limpar os campos da avaliação em andamento?\n\nO cadastro do paciente e as avaliações já registradas no prontuário não serão apagados. Esta ação não pode ser desfeita.',
        { titulo: 'Descartar rascunho', confirmar: 'Descartar rascunho', cancelar: 'Continuar editando', destrutivo: true }
    );
    if (!confirmado) return false;

    const botao = document.getElementById('btn_descartar_rascunho_avaliacao');
    if (botao) botao.disabled = true;
    autosaveSuspensoKineSys = true;
    try {
        const pacienteId = String(pacienteAtualId || document.getElementById('select_paciente_precadastro')?.value || '').trim();
        const lista = pacienteId ? await obterPacientesSalvos() : [];
        const paciente = lista.find(p => String(p.id) === pacienteId) || null;
        ...
        document.querySelectorAll('#tela_avaliacao input, #tela_avaliacao select, #tela_avaliacao textarea').forEach(el => {
            if (el.type === 'checkbox' || el.type === 'radio') el.checked = el.defaultChecked;
                ...
                const padrao = [...el.options].findIndex(opt => opt.defaultSelected);
                el.selectedIndex = padrao >= 0 ? padrao : 0;
            } else el.value = el.defaultValue || '';
        });
        ['tags_cirurgias','tags_medicamentos','grupo_regioes_mapeamento','container_clusters_regioes'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.replaceChildren();
        });
        preencherClinicaEstruturada({});
        const cirurgias = document.getElementById('bloco_cirurgias');
        if (cirurgias) cirurgias.style.display = 'none';
        const realizado = document.getElementById('avaliacao_realizado_em');
        if (realizado) { realizado.disabled = false; realizado.value = valorDatetimeLocalAgora(); }
        const salvar = document.getElementById('btn_salvar_avaliacao_rapida');
        if (salvar) salvar.textContent = '💾 Salvar Avaliação';
        pacienteAtualId = paciente?.id || null;
        if (typeof definirPacienteContexto === 'function') await definirPacienteContexto(paciente?.id || '');
        await atualizarSelectPacientesPreCadastro();
        if (paciente) {
            document.getElementById('select_paciente_precadastro').value = String(paciente.id);
            await carregarPacientePreCadastradoNaAvaliacao();
        }
        fecharBuscaPacientesPreCadastro();
        renderizarRegistrosAvaliacao(paciente);
        irParaSubtela('subtela_triagem');
        fecharRadarKineSys(true);
        processarRadarEmTempoReal();
        if (typeof renderizarAnaliseIrradiacao === 'function') renderizarAnaliseIrradiacao();
        if (typeof atualizarFonteDadosCompacta === 'function') atualizarFonteDadosCompacta();
        alert('✅ Rascunho descartado. Nenhuma avaliação registrada foi apagada.');
        return true;
    } catch (err) {
        console.error('KineSys: não foi possível descartar o rascunho.', err);
        alert('❌ Não foi possível concluir o descarte do rascunho. Tente novamente.');
        return false;
    } finally {
        clearTimeout(autosaveTimerKineSys);
        autosaveTimerKineSys = null;
        ocultarStatusAutosaveKineSys();
        autosaveSuspensoKineSys = false;
...
async function restaurarRascunhoKineSys() {
    ...
    if(!(await confirmarKineSys('Foi encontrado um rascunho de avaliação não finalizada. Deseja restaurá-lo?', {titulo:'Restaurar rascunho', confirmar:'Restaurar'}))) { limparRascunhoKineSys(); return false; }
